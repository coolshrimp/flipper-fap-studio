...
fn start_control_input() {
    std::thread::spawn(|| {
        ...
        for line in stdin.lock().lines().flatten() {
            ...
            if parts.len() == 3 && parts[0] == "BUTTON" {
                let pressed = parts[2] == "PRESS";
                peripherals::gpio::wait_button_ready(
                    parts[1],
                    std::time::Duration::from_secs(10),
                );
                if peripherals::gpio::set_button(parts[1], pressed) {
                    info!("FLIPPER_INPUT {} {}", parts[1], parts[2]);
                }
            }
        }
    });
